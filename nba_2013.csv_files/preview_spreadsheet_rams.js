...
(function() {
    ...
    searchRoot = function() {
        ...
        frameMessenger.configureParentMessaging(
            ...
            ["listening_sfj", "notify_sfj", "print", "enter-fullscreen", "exit-viewer-fullscreen",
             ...
             "keydown-keys-handled-by-parent", "increase-tab-height"]
            );
    ...
    // this function does two things to increase the tab heights in the default excel spreadsheet
    //   1. adds `increased-tabstrip-height` class to all <div> children of the #tabstrip element
    //   2. adds `adjusted-tab-link` class to all anchor tags in the <div> children from (1)
    var increaseTabHeight = function() {
        var tabStripChildren = document.getElementById('tabstrip') && document.getElementById('tabstrip').children;
        var tabStripDivs = Array.prototype.slice.call(tabStripChildren);
        for (var i=0; i<tabStripDivs.length; i++) {
            var tabStripDiv = tabStripDivs[i];
            if (tabStripDiv.tagName && tabStripDiv.tagName.toUpperCase() !== 'DIV') {
                continue;
            }
            tabStripDiv.classList && tabStripDiv.classList.add('increased-tabstrip-height');
            var anchorChildren = tabStripDiv.children;
            var anchors = Array.prototype.slice.call(anchorChildren);
            for (var j=0; j<anchors.length; j++) {
                anchor = anchors[j];
                if (anchor.tagName && anchor.tagName.toUpperCase() !== 'A') {
                    continue;
                }
                anchor.classList && anchor.classList.add('adjusted-tab-link');
            };
        }

        // this resize function is provided by the generated html. it is used to
        // resize table cell height so that it is above the tabs at the bottom
        if (resize) {
            resize();
        }
    }
